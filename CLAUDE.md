...
[`which-skill`](./skills/engineering/which-skill/SKILL.md) is the router over every skill this repo documents — model-invoked as well as user-invoked — and maps how they relate. The same trigger that re-syncs a docs page applies to it: whenever you add, rename, remove, or change how a documented skill fits the flows, re-read `which-skill`'s `SKILL.md` and update it so the map stays accurate — a new skill it never mentions, or a stale one it still routes to, is a router that lies.

To (re)link every skill into the local harness skill directories (`~/.claude/skills`, `~/.agents/skills`), run `scripts/link-skills.sh`. Each entry is a symlink into this repo, so a `git pull` keeps installed skills current; re-run the script after adding, removing, or renaming a skill.

`scripts/checker.py` asserts the invariants above — skill names, docs pages, router completeness, link resolution, invocation pairing — and warns where a skill has been committed more recently than its docs page or the router. Run it any time with `python3 scripts/checker.py`. Structural violations exit non-zero; staleness warnings alone exit zero, so they never block a commit. To gate commits on it, install the hook once per clone (`.git/hooks` is not version-controlled): `ln -sf ../../scripts/pre-commit .git/hooks/pre-commit`.
