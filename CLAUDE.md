...
This is a personal, symlink-installed fork (see `scripts/link-skills.sh`), not a published bundle — there is no marketplace, changeset, or automated release tooling. A minimal `.claude-plugin/plugin.json` is kept only to describe the promoted set for local Claude Code use; run `claude plugin validate . --strict` after touching it. The changelog is hand-maintained.
