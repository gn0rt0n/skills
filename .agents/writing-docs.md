...
Every skill in `engineering/` and `productivity/` has a human-facing **docs page** at `docs/<bucket>/<skill-name>.md` — the docs tree mirrors those two bucket folders under `skills/`. These are **internal reference pages**, read in the repo (e.g. on GitHub); there is no published docs site. The page is not the skill and not a copy of `SKILL.md`. Only these two buckets are promoted; non-promoted buckets (`misc/`, plus any `in-progress/`/`deprecated/`) ship no docs page.
...
Act whenever a promoted skill is added, renamed, or has its behaviour changed: create or re-sync its docs page. A rename moves the file too (`docs/<bucket>/<old>.md` → `docs/<bucket>/<new>.md`); a skill that moves between `engineering/` and `productivity/` moves its docs file to the matching folder. Skills in non-promoted buckets (`misc/`, plus any `in-progress/`/`deprecated/`) get no page. A skill moving *out* of one of them into `engineering/` or `productivity/` gains a page; one moving the other way loses it.

Links between docs pages are **repo-relative**: `./<name>.md` for a sibling in the same bucket, `../<bucket>/<name>.md` across buckets. A link into the source points at the skill directory in this repo.

There is no H1 — the page's identity is its filename.
...
[Source](https://github.com/gn0rt0n/skills/tree/main/skills/<bucket>/<name>)
...
Optional — include only when the skill needs something in place to be functional; omit the heading entirely otherwise. Covers: a **workspace it writes into** (a stateful skill like `grill-with-docs` writes `CONTEXT.md` and ADRs; `teach` builds a whole directory — say what it writes and where), **prior setup** (`triage`/`to-spec`/`to-tickets` need `setup-gn0rt0n-skills` to have configured an issue tracker), or **repo-specific tooling**. A stateless skill that runs anywhere has no prerequisites — drop the section.
...
- **Role.** Name it: a **chain step** (`grill-with-docs → to-spec → to-tickets → implement → code-review`), a **run-once setup** (`setup-gn0rt0n-skills`), **periodic maintenance** (`improve-codebase-architecture`, "every few days"), or a **reach-for-it-anytime standalone** (`diagnosing-bugs`, `prototype`, `handoff`). A standalone's map is one honest sentence — far better than omitting the section.
...
- The source link names the correct bucket and skill.
